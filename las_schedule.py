...
class Business:
    def __init__(self, id, compute_time, data_matrix):
        ...
        self.total_service_time = 0  # 关键字段：累计获得的服务时间
...
def simulate_las_with_starvation(
        businesses,
        network,
        total_simulation_time,
        starvation_threshold=5.0
):
    """LAS调度+饥饿补偿（非抢占式，等待当前业务完成）"""
    current_time = 0.0
    event_queue = []
    waiting_businesses = []  # 最小堆：(total_service_time, 业务ID, 业务对象)
    starving_businesses = []  # 饥饿业务专用堆（同样按LAS排序）

    # 初始化：所有业务加入等待队列，按total_service_time排序
    for biz in businesses:
        heapq.heappush(waiting_businesses, (biz.total_service_time, biz.id, biz))
    ...
    while current_time < total_simulation_time and (event_queue or waiting_businesses or starving_businesses):
        ...
        for biz in current_starving:
            heapq.heappush(starving_businesses, (biz.total_service_time, biz.id, biz))
        ...
        if network.active_business is None:
            # 优先调度饥饿业务中total_service_time最小的
            ...
            # print(f"{'⚠️ 饥饿' if is_starving else '⏳ LAS'}调度 biz_{target_biz.id} | "
            #       f"已服务={target_biz.total_service_time:.2f}s | "
            #       f"等待={current_time - target_biz.last_comm_time:.2f}s")
        ...
        if event_type == 'comm_complete':
            ...
            # 更新累计服务时间（LAS核心逻辑）
            biz.total_service_time += biz.communication_time
            # 重新加入等待队列（按更新后的total_service_time排序）
            heapq.heappush(waiting_businesses, (biz.total_service_time, biz.id, biz))
    ...
    for biz in businesses:
        ...
        #       f"总服务时间={biz.total_service_time:.2f}s | "
...
if __name__ == "__main__":
    ...
    simulate_las_with_starvation(businesses, network, total_simulation_time=100.0)
