    ...
    for i in range(len(job_set_tpe)):
        ...
        for j in range(len(no_link_index_row)):
            ...
            if job_matrix[job_index][row_stuff][col_stuff] == 0:
                ...
                _, path_zip = max_min_weight_path_dijkstra_k_hops(stuff_compose + 10000000 * np.ones_like(stuff_compose), row, col, 2)
                if len(path_zip) == 0:
        ...
        for n in reversed(range(4, 5)):
            ...
            while u < 1:
                ...
                for i in range(len(job_set)):
                    ...
                    avg = sjf_schedule.simulate_sjf_with_starvation(businesses, network, 1000, 5 * m)
                    avg_las = las_schedule.simulate_las_with_starvation(businesses_las, network_las, 1000, 5 * m)
                    avg_hrrn = hrrn_schedule.simulate_hrrn_with_starvation(businesses_hrrn, network_hrrn, 1000, 5 * m)
