        ...
        for n in reversed(range(0, 5)):
            ...
            while u < 5:
                ...
                for i in range(len(job_set)):
                    ...
                    avg = sjf_schedule.simulate_sjf_with_starvation(businesses, network, 1000, 12 + 2 * n)
                    avg_las = las_schedule.simulate_las_with_starvation(businesses_las, network_las, 1000, 12 + 2 * n)
                    avg_hrrn = hrrn_schedule.simulate_hrrn_with_starvation(businesses_hrrn, network_hrrn, 1000, 12 + 2 * n)
                ...
                for k in range(len(result)):
                    ws.append(result[k])
                    wb.save(f"output_{pod_number}_racks.xlsx")
